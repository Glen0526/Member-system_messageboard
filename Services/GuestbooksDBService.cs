...
namespace WebApplication1.Services
{
    public class GuestbooksDBService
    {
        ...
        public List<Guestbooks> GetAllDataList(ForPaging paging)
        {
            ...
            List<Guestbooks> DataList = new List<Guestbooks>();
            //Sql語法
            string sql = $@" select m.*,d.Name,d.IsAdmin from (SELECT row_number() OVER(order by Id) AS sort,
                          * FROM Guestbooks) m inner join Members d on m.Account = d.Account WHERE m.sort 
        ...
        public List<Guestbooks> GetAllDataList(ForPaging paging,string Search)
        {
            ...
            //Sql語法
            string sql = $@" SELECT m.*,d.Name,d.IsAdmin FROM (SELECT row_number() OVER(order by Id) AS sort,
                          * FROM Guestbooks WHERE  Content LIKE '%{Search}%' OR Reply LIKE '%{Search}%') 
