...
namespace WebApplication1.Services
{
    public class MembersDBService
    {
        ...
        public void Register(Members newMember)
        {
            ...
            //sql新增語法//IsAdmin預設為0
            string sql = $@" INSERT INTO Members (Account,Password,Name,Email,AuthCode,IsAdmin) VALUES('{newMember.Account}','{newMember.Password}'
                ,'{newMember.Name}','{newMember.Email}','{newMember.AuthCode}','0')";
            //確保程式不會因執行錯誤而全部中斷
            try
            {
                //開啟資料庫連線
        ...
        private Members GetDataByAccount(string Account)
        {
            ...
            try
            {
                ...
                Data.Name = dr["Password"].ToString();
        ...
        public string EmailValidate(string Account, string AuthCode)
        {
            ...
            if(ValidateMember != null)
            {
                ...
                if(ValidateMember.AuthCode == AuthCode)
                {
                    ...
                    ValidateStr = "帳號信箱驗證成功,可登入囉囉囉囉囉";
                }
                else
                {
                    ValidateStr = "驗證碼錯啦! 重新確認或再註冊唄!";
                }
            }
            else
            {
                ValidateStr = "傳送資料錯誤,重新確認或者在註冊吧";
            }
            //回傳驗證訊息
            return ValidateStr;
        }
